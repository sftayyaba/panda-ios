...
class PNGetDefaultsOperation: OnebyteNetworkOperationBase {
    ...
    override func handleDidFinishedWithResponse(response: AnyObject!) {
        ...
        if let code = json["code"].int{
            ...
            if code >= PNApiResponseCodes.successResponse.rawValue{
                let codeResponseObject: PNUserDefaults = PNUserDefaults(json: JSON(response))
    ...
    private func startLoginOperation() {
        ...
        let urlWithId:String = AppNetworkEndPoints.kGetUserDefaults
