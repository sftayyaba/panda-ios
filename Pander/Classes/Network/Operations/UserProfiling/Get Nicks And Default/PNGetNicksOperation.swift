...
class PNGetNicksOperation: OnebyteNetworkOperationBase {
    ...
    override func handleDidFinishedWithResponse(response: AnyObject!) {
        ...
        if let code = json["code"].int{
            ...
            if code >= PNApiResponseCodes.successResponse.rawValue{
                let codeResponseObject: PNGetNicksResponse = PNGetNicksResponse(json: JSON(response))
    ...
    private func startLoginOperation() {
        ...
        let urlWithId:String = AppNetworkEndPoints.kGetUserNicks
